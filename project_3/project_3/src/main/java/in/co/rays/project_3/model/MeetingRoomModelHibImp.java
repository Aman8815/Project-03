...
public class MeetingRoomModelHibImp  implements MeetingRoomModelInt{
	...
	public long add(MeetingRoomDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		Transaction tx = null;
		long pk = 0;
		MeetingRoomDTO existDto = findByName(dto.getAssetCode());
		if (existDto != null) {
			throw new DuplicateRecordException("assetCode already exist");
		}
		try {
	...
	public MeetingRoomDTO findByPK(long pk) throws ApplicationException {
		...
		try {
			...
			dto = (MeetingRoomDTO) session.get(MeetingRoomDTO.class, pk);
		} catch (HibernateException e) {
	...
	public MeetingRoomDTO findByName(String name) throws ApplicationException {
		...
		try {
			session = HibDataSource.getSession();
			Criteria criteria = session.createCriteria(MeetingRoomDTO.class);
			criteria.add(Restrictions.eq("assetCode", name));
			List list = criteria.list();
			if (list.size() > 0) {
				dto = (MeetingRoomDTO) list.get(0);
			}
		} catch (HibernateException e) {
