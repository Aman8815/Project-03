 ...
 *
 */
@WebServlet(name = "ScheduleListCtl", urlPatterns = { "/ctl/ScheduleListCtl" })
public class ScheduleListCtl extends BaseCtl {
	private static Logger log = Logger.getLogger("ScheduleListCtl.class");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				ServletUtility.redirect(ORSView.SCHEDULE_CTL, request, response);
				return;
			} else if (OP_RESET.equalsIgnoreCase(op)) {

				ServletUtility.redirect(ORSView.SCHEDULE_LIST_CTL, request, response);
				return;
				...
				if (ids != null && ids.length > 0) {
					ScheduleDTO deletebean = new ScheduleDTO();
					for (String id : ids) {
			...
			dto = (ScheduleDTO) populateDTO(request);
			list = model.search(dto, pageNo, pageSize);
			ServletUtility.setDto(dto, request);
			 next = model.search(dto, pageNo + 1, pageSize);
			 ServletUtility.setList(list, request);
	...
	protected String getView() {
		...
		return ORSView.SCHEDULE_LIST_VIEW;
	}
