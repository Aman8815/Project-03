 ...
 *
 */
@WebServlet(name = "BudgetListCtl", urlPatterns = { "/ctl/BudgetListCtl" })
public class BudgetListCtl extends BaseCtl {
	private static Logger log = Logger.getLogger("BudgetListCtl.class");
		...
		try {
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					BudgetDTO deletebean = new BudgetDTO();
					for (String id : ids) {
			...
			dto = (BudgetDTO) populateDTO(request);
			list = model.search(dto, pageNo, pageSize);
			ServletUtility.setDto(dto, request);
			 next = model.search(dto, pageNo + 1, pageSize);
			 ServletUtility.setList(list, request);
	...
	protected String getView() {
		...
		return ORSView.BUDGET_LIST_VIEW;
	}
