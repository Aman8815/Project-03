...
public class FeeModelHibImp implements FeeModelInt  {
	...
	public long add(FeeDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		Transaction tx = null;
		long pk = 0;
		FeeDTO existDto = findByName(dto.getStudentId());
		if (existDto != null) {
	...
	public FeeDTO findByPK(long pk) throws ApplicationException {
		...
		try {
			...
			dto = (FeeDTO) session.get(FeeDTO.class, pk);
		} catch (HibernateException e) {
		  ...
		  Session session = null;
	        ...
	        try {
	            ...
	            if (dto.getPaymentDate() != null && dto.getPaymentDate().getTime() > 0) {
	                criteria.add(Restrictions.like("paymentDate", dto.getPaymentDate()));
	            }
