...
public class SmartParkingModelHibImp implements SmartParkingModelInt  {
	...
	public long add(SmartParkingDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		Transaction tx = null;
		long pk = 0;
		SmartParkingDTO existDto = findByName(dto.getParkingCode());
		if (existDto != null) {
			throw new DuplicateRecordException("ParkingCode already exist");
		}
		try {
	...
	public SmartParkingDTO findByName(String name) throws ApplicationException {
		// TODO Auto-generated method stub
		Session session = null;
		SmartParkingDTO dto = null;
		try {
			...
			criteria.add(Restrictions.eq("parkingCode", name));
		...
		// TODO Auto-generated method stub
		  Session session = null;
	        ...
	        try {
	            ...
	            if (dto.getParkingCode() != null && dto.getParkingCode().length() > 0) {
	                criteria.add(Restrictions.like("parkingCode", dto.getParkingCode() + "%"));
	            }
	            if (dto.getSlotNumber() != null && dto.getSlotNumber().length()>0) {
	                criteria.add(Restrictions.eq("slotNumber", dto.getSlotNumber()+"%"
	                    ));
	            }
