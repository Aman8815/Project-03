...
public class InsuranceAppModelHibImp implements InsuranceAppModelInt  {
	...
	public long add(InsuranceAppDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		Transaction tx = null;
		long pk = 0;
		InsuranceAppDTO existDto = findByName(dto.getCustomerName());
		if (existDto != null) {
			throw new DuplicateRecordException("CustomerName already exist");
		}
		try {
	...
	public InsuranceAppDTO findByPK(long pk) throws ApplicationException {
		...
		Session session = null;
		InsuranceAppDTO dto = null;
		try {
			session = HibDataSource.getSession();

			dto = (InsuranceAppDTO) session.get(InsuranceAppDTO.class, pk);
		} catch (HibernateException e) {
	...
	public InsuranceAppDTO findByName(String name) throws ApplicationException {
		...
		try {
			session = HibDataSource.getSession();
			Criteria criteria = session.createCriteria(InsuranceAppDTO.class);
			criteria.add(Restrictions.eq("customerName", name));
			List list = criteria.list();
			if (list.size() > 0) {
				dto = (InsuranceAppDTO) list.get(0);
			}
		} catch (HibernateException e) {
		...
		// TODO Auto-generated method stub
		  Session session = null;
	        ...
	        try {
	            ...
	            if (dto.getClaimStatus() != null && dto.getClaimStatus().length() > 0) {
	                criteria.add(Restrictions.eq("claimStatus", dto.getClaimStatus()+"%"
	                    ));
	            }
	            if (dto.getPolicyType() != null && dto.getPolicyType().length() > 0) {
	                criteria.add(Restrictions.like("policyType", dto.getPolicyType()+"%"));
	            }
