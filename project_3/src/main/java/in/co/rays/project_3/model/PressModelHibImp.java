...
public class PressModelHibImp implements PressModelInt  {
	...
	public long add(PressDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		Transaction tx = null;
		long pk = 0;
		PressDTO existDto = findByName(dto.getAuthor());
		if (existDto != null) {
	...
	public PressDTO findByPK(long pk) throws ApplicationException {
		...
		try {
			...
			dto = (PressDTO) session.get(PressDTO.class, pk);
		} catch (HibernateException e) {
	...
	public PressDTO findByName(String name) throws ApplicationException {
		...
		try {
			session = HibDataSource.getSession();
			Criteria criteria = session.createCriteria(PressDTO.class);
			criteria.add(Restrictions.eq("author", name));
			List list = criteria.list();
			if (list.size() > 0) {
				dto = (PressDTO) list.get(0);
			}
		} catch (HibernateException e) {
		  ...
		  Session session = null;
	        ...
	        try {
	            ...
	            if (dto.getAuthor() != null && dto.getAuthor().length() > 0) {
	                criteria.add(Restrictions.eq("author", dto.getAuthor()+"%"
	                    ));
	            }
	            if (dto.getReleaseDate() != null && dto.getReleaseDate().getTime() > 0) {
	                criteria.add(Restrictions.like("releaseDate", dto.getReleaseDate()));
	            }
