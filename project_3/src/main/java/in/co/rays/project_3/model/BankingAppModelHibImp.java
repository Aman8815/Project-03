...
public class BankingAppModelHibImp implements BankingAppModelInt  {
	...
	public long add(BankingAppDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		Transaction tx = null;
		long pk = 0;
		BankingAppDTO existDto = findByName(dto.getBranchName());
		if (existDto != null) {
			throw new DuplicateRecordException("Branch Name already exist");
		}
		try {
	...
	public BankingAppDTO findByPK(long pk) throws ApplicationException {
		...
		Session session = null;
		BankingAppDTO dto = null;
		try {
			session = HibDataSource.getSession();

			dto = (BankingAppDTO) session.get(BankingAppDTO.class, pk);
		} catch (HibernateException e) {
	...
	public BankingAppDTO findByName(String name) throws ApplicationException {
		...
		try {
			session = HibDataSource.getSession();
			Criteria criteria = session.createCriteria(BankingAppDTO.class);
			criteria.add(Restrictions.eq("branchName", name));
			List list = criteria.list();
			if (list.size() > 0) {
				dto = (BankingAppDTO) list.get(0);
			}
		} catch (HibernateException e) {
		...
		// TODO Auto-generated method stub
		  Session session = null;
	        ...
	        try {
	            ...
	            if (dto.getBalance() != 0 && dto.getBalance() > 0) {
	                criteria.add(Restrictions.eq("balance", dto.getBalance()
	                    ));
	            }
	            if (dto.getIFSCCode() != null && dto.getIFSCCode().length() > 0) {
	                criteria.add(Restrictions.like("IFSCCode", dto.getIFSCCode()));
	            }
