 ...
 *
 */
@WebServlet(name = "BankingAppListCtl", urlPatterns = { "/ctl/BankingAppListCtl" })
public class BankingAppListCtl extends BaseCtl {
	private static Logger log = Logger.getLogger("BankingAppListCtl.class");
		...
		try {
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					BankingAppDTO deletebean = new BankingAppDTO();
					for (String id : ids) {
			...
			dto = (BankingAppDTO) populateDTO(request);
			list = model.search(dto, pageNo, pageSize);
			ServletUtility.setDto(dto, request);
			 next = model.search(dto, pageNo + 1, pageSize);
			 ServletUtility.setList(list, request);
	...
	protected String getView() {
		...
		return ORSView.BANKINGAPP_LIST_VIEW;
	}
