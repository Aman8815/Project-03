 ...
 *
 */
@WebServlet(name = "ScholarshipListCtl", urlPatterns = { "/ctl/ScholarshipListCtl" })
public class ScholarshipListCtl extends BaseCtl {
	private static Logger log = Logger.getLogger("ScholarshipListCtl.class");
		...
		try {
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					ScholarshipDTO deletebean = new ScholarshipDTO();
					for (String id : ids) {
			...
			dto = (ScholarshipDTO) populateDTO(request);
			list = model.search(dto, pageNo, pageSize);
			ServletUtility.setDto(dto, request);
			 next = model.search(dto, pageNo + 1, pageSize);
			 ServletUtility.setList(list, request);
	...
	protected String getView() {
		...
		return ORSView.SCHOLARSHIP_LIST_VIEW;
	}
