...
public class PetrolPumpModelHibImp implements PetrolPumpModelInt  {
	...
	public long add(PetrolPumpDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		Transaction tx = null;
		long pk = 0;
		PetrolPumpDTO existDto = findByName(dto.getCustomerName());
		if (existDto != null) {
	...
	public PetrolPumpDTO findByPK(long pk) throws ApplicationException {
		...
		Session session = null;
		PetrolPumpDTO dto = null;
		try {
			session = HibDataSource.getSession();

			dto = (PetrolPumpDTO) session.get(PetrolPumpDTO.class, pk);
		} catch (HibernateException e) {
	...
	public PetrolPumpDTO findByName(String name) throws ApplicationException {
		...
		try {
			session = HibDataSource.getSession();
			Criteria criteria = session.createCriteria(PetrolPumpDTO.class);
			criteria.add(Restrictions.eq("customerName", name));
			List list = criteria.list();
			if (list.size() > 0) {
				dto = (PetrolPumpDTO) list.get(0);
			}
		} catch (HibernateException e) {
		...
		// TODO Auto-generated method stub
		  Session session = null;
	        ...
	        try {
	            ...
	            if (dto.getLiters() != 0 && dto.getLiters() > 0) {
	                criteria.add(Restrictions.eq("loanAmount", dto.getLiters()
	                    ));
	            }
	            if (dto.getFuelType() != null && dto.getFuelType().length() > 0) {
	                criteria.add(Restrictions.like("fuelType", dto.getFuelType()+"%"));
	            }
