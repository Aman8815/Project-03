...
public class SmartModelHibImp implements SmartModelInt  {
	...
	public long add(SmartDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		Transaction tx = null;
		long pk = 0;
		SmartDTO existDto = findByName(dto.getLightCode());
		if (existDto != null) {
			throw new DuplicateRecordException("lightCode already exist");
		}
		try {
	...
	public SmartDTO findByPK(long pk) throws ApplicationException {
		...
		try {
			...
			dto = (SmartDTO) session.get(SmartDTO.class, pk);
		} catch (HibernateException e) {
	...
	public SmartDTO findByName(String name) throws ApplicationException {
		...
		try {
			session = HibDataSource.getSession();
			Criteria criteria = session.createCriteria(SmartDTO.class);
			criteria.add(Restrictions.eq("lightCode", name));
			List list = criteria.list();
			if (list.size() > 0) {
				dto = (SmartDTO) list.get(0);
			}
		} catch (HibernateException e) {
		  ...
		  Session session = null;
	        ...
	        try {
	            ...
	            if (dto.getRoomName() != null && dto.getRoomName().length() > 0) {
	                criteria.add(Restrictions.eq("roomName", dto.getRoomName()+"%"
	                    ));
	            }
