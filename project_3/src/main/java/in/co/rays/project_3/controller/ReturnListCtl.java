 ...
 *
 */
@WebServlet(name = "ReturnListCtl", urlPatterns = { "/ctl/ReturnListCtl" })
public class ReturnListCtl extends BaseCtl {
	private static Logger log = Logger.getLogger("ReturnListCtl.class");
		...
		try {
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					ReturnDTO deletebean = new ReturnDTO();
					for (String id : ids) {
			...
			dto = (ReturnDTO) populateDTO(request);
			list = model.search(dto, pageNo, pageSize);
			ServletUtility.setDto(dto, request);
			 next = model.search(dto, pageNo + 1, pageSize);
			 ServletUtility.setList(list, request);
	...
	protected String getView() {
		...
		return ORSView.RETURN_LIST_VIEW;
	}
