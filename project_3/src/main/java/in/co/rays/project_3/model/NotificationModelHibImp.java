...
public class NotificationModelHibImp implements NotificationModelInt  {
	...
	public long add(NotificationDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		Transaction tx = null;
		long pk = 0;
		NotificationDTO existDto = findByName(dto.getTemplateCode());
		if (existDto != null) {
			throw new DuplicateRecordException("TemplateCode already exist");
		}
		try {
	...
	public NotificationDTO findByName(String name) throws ApplicationException {
		...
		try {
			...
			criteria.add(Restrictions.eq("templateCode", name));
		...
		// TODO Auto-generated method stub
		  Session session = null;
	        ...
	        try {
	            ...
	            if (dto.getTemplateCode() != null && dto.getTemplateCode().length() > 0) {
	                criteria.add(Restrictions.like("templateCode", dto.getTemplateCode() + "%"));
	            }
